<template>
    ...
    >
            ...
            <v-toolbar flat color="blue lighten-1">
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    <template v-slot:activator="{ on }">
                        <v-btn   color="blue darken-4" fab dark class="mb-2" v-on="on">
                            <v-icon dark>+</v-icon>
                        </v-btn>
                    </template>
                    <v-card>
                        ...
                        <v-card-actions class="blue darken-3">
                            <v-spacer></v-spacer>
                            <v-btn color="light-blue lighten-4" text @click="close">Cancelar</v-btn>
                            <v-btn color="light-blue lighten-4" text @click="guardar">Guardar</v-btn>
                        </v-card-actions>
                    </v-card>
                </v-dialog>
            </v-toolbar>
        ...
        <template v-slot:item.editar="{ item }">
            <v-icon small class="mr-2" @click="editarEstudiante(item)">
                mdi-pencil
            </v-icon>
        </template>
        <template v-slot:item.eliminar="{ item }">
            <v-icon small @click="eliminarEstudiante(item)">
                mdi-delete
            </v-icon>
        </template>
    </v-data-table>
    ...
    export default {
        ...
        methods: {
            ...
            async eliminarEstudiante(item) {
                ...
                let confirmacion =
                    confirm("¿Realmente deseas eliminar a este Estudiante?") &&
